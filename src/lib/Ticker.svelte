...
<div class="ticker">
    ...
    {#each newData as node}
        ...
        <div class="rounded-r-xl">{Math.round(node.y)}</div>
    {/each}
</div>

<style lang="postcss">
    .ticker {
        @apply my-8 pr-8 grid grid-cols-5 bg-base-100;
    }
    ...
    .ticker h3 {
        @apply mb-4 py-4 text-center text-2xl font-bold uppercase sticky top-0 bg-base-100;
    }
</style>
