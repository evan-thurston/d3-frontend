...
<div class="controls">
    ...
    <button
        class={!open || currentView !== 0 ? 'primaryButton' : 'disabledButton'}
    ...
    <!-- <button
        class={open && currentView !== 1 ? 'primaryButton' : 'disabledButton'}
        on:click={() => (tickerPaused = !tickerPaused)}
    >
        {#if tickerPaused}
            <Play />
        {:else}
...
<style lang="postcss">
    .controls {
        ...
        flex flex-row 2xl:flex-col space-x-4 2xl:space-x-0 2xl:space-y-4 top-64
